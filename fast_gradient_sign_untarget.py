...
class FastGradientSignUntargeted():
    ...
    """
    def __init__(self, model, gan, epsilon, alpha, min_val, max_val, max_iters, _type='linf'):
        self.model = model
        self.gan = gan
        self.model.eval()
    ...
    def perturb(self, original_images, labels, reduction4loss='mean', random_start=False,defensive=False,gan=False):
        ...
        with torch.enable_grad():
            for _iter in range(self.max_iters):
                if gan == True:
                    def_pert = self.gan((x - 0.5) * 2)
                    def_x = def_pert 
                    #def_x = (nn.Tanh()(def_x)+1) /2
                    def_x = (def_x + 1 ) / 2
                    #x = x.clamp(0,1).float()
                    outputs = self.model(def_x)
                else:
                ...
                if defensive == False:
                    if self._type == 'l2':
                        ...
                        l = len(x.shape) - 1
                        grad_norm = torch.norm(grads.view(grads.shape[0], -1), dim=1).view(-1, *([1]*l))
                        scaled_grad = grads / (grad_norm + 1e-10)
                        x.data += self.alpha * scaled_grad
